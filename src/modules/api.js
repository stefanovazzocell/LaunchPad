		...
		query('DELETE FROM `links` WHERE `clicks` < 2 OR `expiration` <= NOW();\n' + 
			...
			function(results) {
				...
				if (results[1].length < 1) {
					...
					if (serverOptions.hasOwnProperty('d') && String(serverOptions['d']).length === 0) userOptions.push('_d'); // Public Delete
					if (serverOptions.hasOwnProperty('s') && String(serverOptions['s']).length === 0) userOptions.push('_s'); // Public Stats
